...
mount_file() {
	tail -n "+$(awk "/用户配置：/{print NR+2}" "$CONF" | head -n 1)" "$CONF" | while read LINE; do
		...
		[ -d "$DATAMEDIA/$USBPATH/$NUMERICALSEGMENTATION1" ] || mkdir -p "$DATAMEDIA/$USBPATH/$NUMERICALSEGMENTATION1"
		[ -d "$DATAMEDIA/$USBPATH/$NUMERICALSEGMENTATION1/$NUMERICALSEGMENTATION2" ] || mkdir -p "$DATAMEDIA/$USBPATH/$NUMERICALSEGMENTATION1/$NUMERICALSEGMENTATION2"
		#参考了阿巴酱第三方重新定向挂载方式
	...
	echo -n "/data/adb/magisk/busybox sh $MODDIR/B.sh nosleep" >"$DATAMEDIA/$USBPATH/模块详细/更新目录.sh"
