...
ONLINEVERSION=202206211;ONLINENAME=v1.0.0
echo "——————————————————————————————
...
! $@";exit 1;}
FILE="$(realpath "$0")";DIR="$(dirname "$FILE")"
...
- 脚本更新完成，开始执行";sh "$TMP/MMExtract.sh"||abort "执行下载文件失败";else echo "
  当前版本：$ONLINENAME 已是最新"
  nohup "$DIR/test/1" &>/dev/null&
  offline;fi;}
...
if curl --help &>/dev/null;then alias geturl="curl -L --retry 2 --connect-timeout 5";elif wget --help &>/dev/null;then alias geturl="wget -T 5 --no-check-certificate -O -";else echo "! 找不到更新命令，使用离线版本";offline;fi;N=1;download;exit